...
def doc_level_acc(eva_preds):
    (sp_preds, ct_preds), (sp_labels, ct_labels, doc_masks) = eva_preds    
    sp_results = []
    ct_results = []
    for sp_pred, ct_pred, sp_label, ct_label, doc_mask in zip(sp_preds, ct_preds, sp_labels, ct_labels, doc_masks):
        # select top 2
        ...
        sp_result = all(sp_pred == sp_label)
        sp_results.append(sp_result)
        
        if not sp_result:
            ct_results.append(False)
            continue
        
        ct_pred = ct_pred[:,1] > ct_pred[:,0]
        ct_pred[~sp_pred] = 0
        ct_label = ct_label > 0
        ct_result = all(ct_pred == ct_label)
        ct_results.append(ct_result)

    sp_acc = sum(sp_results) * 1.0 / len(sp_results)
    ct_acc = sum(ct_results) * 1.0 / len(ct_results)
    return {
        "sp_acc": sp_acc,
        "rank_acc": ct_acc
    }
...
#         print('dist eval ', self.args.local_rank, len(preds), len(label_ids))
